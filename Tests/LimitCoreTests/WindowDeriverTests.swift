...
struct ProjectorTests {
    ...
    func linearProjection() throws {
        ...
        let state = try #require(deriver.derive(.fiveHour, from: data, now: now))
        let p = try #require(projector.project(state, samples: data, now: now))
    ...
    func flatUsagePushesFillLater() throws {
        ...
        let earlyProjection = try #require(projector.project(
            earlyState, samples: early, now: now.addingTimeInterval(-120 * 60)))
        ...
        let laterProjection = try #require(projector.project(laterState, samples: later, now: now))
    ...
    @Test("Uzun boşluklar taban hıza karışmaz")
    func baselineIgnoresGaps() {
        // 6 saatlik boşluktan sonra %5 artış, gerçek çalışma temposu değil.
        let data = samples([(400, 10, 10), (40, 15, 12), (30, 25, 14), (0, 45, 20)], now: now)
        let base = projector.baselineRate(.fiveHour, samples: data, now: now)
        if let base { #expect(base > 5) }  // yalnızca kısa aralıklar sayıldı
    }
}
