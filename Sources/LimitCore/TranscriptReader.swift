...
struct ActivityEvent: Sendable, Equatable {
    let date: Date
    let model: String
    let inputTokens: Int
...
private final class ResultSink: Sendable {
    typealias Entry = (key: String, event: ActivityEvent)

    private let storage: Mutex<[Entry]>

    init() {
        storage = Mutex([])
        storage.withLock { $0.reserveCapacity(64 << 10) }
    }

    func append(_ items: [Entry]) {
        guard !items.isEmpty else { return }
        storage.withLock { $0.append(contentsOf: items) }
    }
...
public struct TranscriptReader: Sendable {
    public static var defaultRoot: URL {
        StoragePaths.claudeCodeProjects
    }

    let root: URL

    public init(root: URL = TranscriptReader.defaultRoot) {
        self.root = root
    }
