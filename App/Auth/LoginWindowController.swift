...
final class LoginWindowController: NSObject, NSWindowDelegate {
    ...
    func present() {
        ...
        webView.load(URLRequest(url: ClaudeWebClient.loginURL))
    ...
    private func checkForSession() {
        ...
        webView.configuration.websiteDataStore.httpCookieStore.getAllCookies { [weak self] cookies in
            ...
            guard let cookie = cookies.first(where: {
                ...
                // gibi bir alan adı `contains` denetimini geçer.
                $0.name == "sessionKey" && ClaudeWebClient.isClaudeCookieDomain($0.domain)
            }) else { return }
            // Çıkış yapıldığında çerez boş bir değerle yeniden yazılabiliyor;
            // gerçek anahtar her zaman `sk-ant-` ile başlıyor.
            guard ClaudeWebClient.isPlausibleSessionKey(cookie.value) else { return }
    ...
    private func finish(with sessionKey: String, expiresAt: Date?) {
        ...
        pollTimer = nil
        // Son kullanma tarihi "oturum N gün sonra dolacak" uyarısının tek
        // kaynağı; atılmamalı.
...
extension LoginWindowController: WKNavigationDelegate {
    /// Sayfa hiç açılamadığında (ağ yok, DNS düşmüş ya da claude.ai
    /// erişilemiyor) kullanıcı boş beyaz bir pencere yerine bir açıklama görür.
    func webView(
        _ webView: WKWebView,
    ...
    private func showLoadFailure(_ error: Error) {
        ...
        alert.beginSheetModal(for: window) { [weak self] response in
            ...
            if response == .alertFirstButtonReturn {
                self.webView?.load(URLRequest(url: ClaudeWebClient.loginURL))
            } else {
                window.close()
            }
        }
...
extension LoginWindowController: WKUIDelegate {
    ...
    func webView(
        ...
        window.title = L.t("Giriş", "Sign in")
