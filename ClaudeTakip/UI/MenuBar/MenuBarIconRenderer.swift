...
final class MenuBarIconRenderer {
    /// Brand asterisk (#D97757) and the system-status alert variant (#E04443).
    private let logoImage: NSImage?
    private let alertLogoImage: NSImage?

    // Brand asterisk fill, and the alert fill used when Claude's system
    // status is not operational.
    private static let brandFill = "#D97757"
    private static let alertFill = "#E04443"

    // Official Claude asterisk SVG path (viewBox 0 0 24 24). The fill color
    // is templated so the same path can render brand or alert-tinted.
    private static func claudeSVG(fill: String) -> String {
        """
        ...
        18.56l-1.13.146-.487-.456.061-.746.231-.243 1.908-1.312-.006.006z" fill="\(fill)" \
        fill-rule="nonzero"/></svg>
        """
    }

    private static func makeLogo(fill: String) -> NSImage? {
        guard let data = claudeSVG(fill: fill).data(using: .utf8),
              let logo = NSImage(data: data) else {
            return nil
        }
    ...
    init() {
        self.logoImage = Self.makeLogo(fill: Self.brandFill)
        self.alertLogoImage = Self.makeLogo(fill: Self.alertFill)
    }

    /// Menu bar: [Claude logo] [pil bar + %XX] [H:MM]
    /// `statusAlert` swaps the brand asterisk for the #E04443 alert variant
    /// when Claude's system status is not operational.
    func render(remaining: Double, resetTimeText: String, hasLoaded: Bool, statusAlert: Bool = false) -> NSImage {
        ...
        // 1. Claude logo — alert-tinted when system status is not operational.
        if let logo = statusAlert ? (alertLogoImage ?? logoImage) : logoImage {
